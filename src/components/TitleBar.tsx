...
export const TitleBar = memo(function TitleBar() {
  ...
  const switchTab = (next: Tab) => {
    if (next === tab) return;
    const { leftOpen, rightPid, archiveOpen, statsDetail } = getUi();
    if (leftOpen || rightPid !== null || archiveOpen || statsDetail) {
      // 面板/子页开着：先收（140ms）再滑
      setUi({ leftOpen: false, rightPid: null, archiveOpen: false, statsDetail: null });
