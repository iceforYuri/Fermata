...
## 2026-09-22 · v1.3 商讨定稿

### D56 · 老化口径改当前段（废止 D4 的累计口径与等AI 剔除）
**决策**：`q_suspended_ms` 从"当天 suspended 且非 waiting_ai 区间的累计和"改为**当前这段挂起**——距上一次挂上（process_create / switch_out / process_reopen）至今，捞回（switch_in / complete）即清零；等AI 是挂起子状态，时段照算。呈现侧等AI 照旧不参与（不褪色、标签不加深）。
**理由**：用户定稿——"挂 xx"回答的是"你多久没碰它了"，累计口径会让反复捞放的进程背上旧账；D4 的等AI 剔除源自对 CONTEXT「不参与老化褪色」的统计层误读，褪色是呈现不是计时。m0 验收文档里的累计数字（8h05m）为历史快照，不回改。

### D57 · 挂起行点击 = 断点卡 + 详情栏同开（废止 D12）
**决策**：点击挂起行同时展开断点卡（贴被点行下方）与右侧详情栏（钉在被点进程）；Enter 完成切换详情不跳，Esc 取消详情也留。
**理由**：用户定稿——D12 的保守选择导致"不激活进程就无法改步骤"，详情面板本就支持任意 pid，只是缺入口；同开后入口成立且不牺牲切换主流程。
