...
| --- | --- |
| 断点 | 放下前留一句话，回来从原处接着做 |
| 单活跃版面 | 折线上永远只有一件事；等得越久"挂"字越沉 |
| 调度即计时 | 切换、休息、空闲、退出自动停表 |
| 休止符 | 到点提醒休息，不催不超时 |
| 三视角统计 | 年 / 月 / 日，日精确到 10 分钟一格 |
| 本地优先 | 无账号无云无遥测，数据全在本机 |

## 与顺手的工具们对照

这些工具都是好工具，只是回答的问题不一样。任务清单（Things）管"该做什么"，手动计时器（Toggl）管"记了多久"，被动追踪（RescueTime / ActivityWatch）管"你当时在哪个应用里"，番茄钟管"节奏"——它们都不管"此刻在做哪件、做到哪一步了"。Fermata 管这个：断点、挂起与恢复、折线上永远只有一件事。

我们不打算做的：多端同步、手机端、团队协作、自动识别你在用什么应用——想要这些的话，上面几位确实更合适。
...
| 形态 | 文件 |
| --- | --- |
| 安装包（推荐） | `publish/Fermata-1.3.0-setup.exe` |
| 绿色单文件 | `publish/Fermata-1.3.0-portable.exe` |
